...
function showtmp(curl,caction){
  ...
  t_img.setAttribute("style","cursor:pointer; cursor:hand");
...
{ 	
 ...
 switch(ftempID)
 {
    ...
    case "selectedinfix" : showSI();return;
...
function showSI()
{
 ...
 for(var j;j<(siElements[0].getElementsByTagName("action").length-1);j++)
 {
  inputbox.setAttribute(siElements[0].getElementsByTagName("action")[j].getAttribute("evt"),siElements[0].getElementsByTagName("action")[j].firstChild.nodeValue);
 }
 var img=showtmp(siElements[0].getElementsByTagName("imgurl")[0].firstChild.nodeValue,siElements[0].getElementsByTagName("action")[2].firstChild.nodeValue);
